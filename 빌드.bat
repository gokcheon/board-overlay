...
echo  ※ 평소엔 올리기.bat만 쓰면 돼요 (깃허브가 대신 빌드) - 이 파일은 내 컴에서 직접 만들 때용
echo.
...
copy /y 패치노트_fix*.txt "dist\BoardOverlay\" >nul 2>nul
copy /y 업데이트주소.txt "dist\BoardOverlay\" >nul 2>nul
copy /y 자동업데이트_안내.txt "dist\BoardOverlay\" >nul 2>nul
copy /y 이용안내.txt "dist\BoardOverlay\" >nul 2>nul
copy /y 메뉴얼.html "dist\BoardOverlay\" >nul 2>nul
...
:haveisl
rem 버전은 server\version.py 한 곳에서 읽어 설치마법사에 주입 (fix28)
set "APPVER=1.0"
for /f %%v in ('python -m server.version') do set "APPVER=%%v"
"%ISCC%" /Q /DAppVer=%APPVER% 설치마법사.iss
if errorlevel 1 goto :err

echo.
echo  ════════════════════════════════════════════════
echo   완성! - 배포용\보드게임오버레이_설치.exe (버전 %APPVER%)
