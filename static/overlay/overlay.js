...
(function () {
  ...
  function applyState(s) {
    ...
    const hist = $("sbHist");
    hist.innerHTML = s.last_rolls
      .map(r => `<span><b>${esc(r.nick)}</b> · ${r.value} → ${esc(r.label)}</span>`)
      .join("");
    hist.style.display = s.last_rolls.length ? "" : "none";
    ...
    if (s.running && s.statusbar_visible) {
      ...
      const per = (s.players || []).map((name, i) => {
        ...
        const sh = g > 0 ? ` <span class="sh">${g <= 3 ? "🛡️".repeat(g) : `🛡️×${g}`}</span>` : "";  // VS16 — 컬러 이모지 강제
        const img = pimg(i);
        const dot = img
      ...
      // fix29: 적립 누계임이 한눈에 보이게 (곡천 확정 문구)
      const snack = (s.counters["안주"] ?? 0) > 0
        ? `<span>🍢 안주 <b>${s.counters["안주"]}</b>개 적립중</span>` : "";
      bar.innerHTML = `<span>🍺 현재 <b>${s.counters["적립"] ?? 0}</b>잔 적립중</span>` + snack + per;
    } else {
      bar.hidden = true;
    }
  ...
  function runRollAnim(anim) {
    ...
    setTimeout(() => {                   // 약 0.8s 롤링 후 값 확정
      ...
      $("sbDest").innerHTML = anim.x2_used
        ? `굴림 <b>${anim.value}</b> ×2 = ${anim.moved}칸!`
        : `굴림 <b>${anim.value}</b>`;
      startHop();
    }, reduced ? 150 : 800);
  }
  ...
  function runAnim(anim) {
    ...
    else if (anim.kind === "guard_gift") showToast(`${anim.nick}님이 방어권을 선물!`);
