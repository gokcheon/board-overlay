...
(function () {
  ...
  // ── 칸 통이미지 (fix42) — control.js 와 같은 규격을 써야 미리보기가 맞는다 ──
  // 그림 캔버스 = 칸 크기 + 사방 TILE_IMG_MARGIN. 칸을 캔버스 한가운데에 맞춰 얹는다.
  // .tile 은 border-box 라 절대배치 자식의 기준은 padding box(테두리 3px 안쪽)다.
  // 그래서 테두리까지 정확히 덮으려면 -(여백 + 테두리 3px) 만큼 밀어야 한다.
  const TILE_IMG_MARGIN = 60;
  const TILE_BORDER = 3;
  let tileSkin = "basic";               // 칸 꾸미기 방식 — "basic" | "image"
  ...
  // 칸끼리 겹치는 순서 (fix42) — 화면 아래쪽 칸이 위, 같은 줄이면 오른쪽 칸이 위.
  // 시안에서 주사위·강아지가 위 칸을 덮고 있던 그 순서다 (곡천 확정).
  // ※ 기본 방식일 땐 아예 안 건다 — 지금까지의 그림 순서를 그대로 두기 위해서.
  ...
  function applyTileSkin(skin) {
    ...
    document.body.classList.toggle("skin-image", tileSkin === "image");
  ...
  function connect() {
    ...
    ws.onmessage = ev => {
      ...
      if (msg.type === "init") {
        ...
        applyTileSkin(msg.config.tile_skin);                                // fix42 (renderBoard 전에)
        renderBoard();
        busy = false;
        ...
        applyTileSkin(msg.tile_skin);                                   // fix42: 칸 꾸미기 방식 즉시 반영
