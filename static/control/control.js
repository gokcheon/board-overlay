...
(function () {
  ...
  function render() {
    ...
    $("stSnack").textContent = s.counters["안주"] ?? 0;
    ...
    $("stHist").innerHTML = s.last_rolls
      .map(r => `<span><b>${esc(r.nick)}</b> · ${r.value ?? "선택"} → ${esc(r.label)}</span>`)
      .join("");
    ...
    if (qJson !== lastQueueJson) {
      ...
      if (s.queue.length === 0) {
        ...
        list.innerHTML = s.queue.map(q => {
          ...
          return `<li><span class="av">${esc(initial)}</span><span class="nm">${esc(q.nick)}</span><b>${q.rolls_left}굴림</b></li>`;
        }).join("");
      }
    }
