...
(function () {
  ...
  async function fetchUserStickers() {
    ...
    } catch (e) { userStickers = []; }
  }

  function buildGrid() {
    const mine = userStickers.map(f =>
      `<span class="stk-cell mine"><button type="button" class="pickimg" data-img="${esc(f)}" title="${esc(f)}">
        <img src="/userstickers/${encodeURIComponent(f)}" alt=""></button>
        <button type="button" class="stk-del" data-del="${esc(f)}" title="이 그림 삭제">✕</button></span>`).join("");
    const base = STICKERS.map(([id, nm]) =>
  ...
  async function openPicker(slot) {
    pickSlot = slot;
    $("stkPickTitle").textContent = `그림 고르기 — ${SLOTS.find(x => x[0] === slot)[1]}`;
  ...
  $("stkFile").addEventListener("change", () => {
    const f = $("stkFile").files[0];
    $("stkFile").value = "";
    if (!f) return;
    if (f.size > 2 * 1024 * 1024) { showMsg("그림이 2MB를 넘어요. 조금 줄여서 올려 주세요."); return; }
    const reader = new FileReader();
    reader.onload = async () => {
      const res = await post("/api/stickers/upload", { name: f.name, data: reader.result });
      if (res.ok) {
        showMsg(`'${res.file}' 업로드 완료 — 눌러서 붙여 보세요.`);
        await fetchUserStickers(); buildGrid();
      }
    };
  ...
  async function renderImages() {
    ...
    userStickers = files;   // 스티커/말 그림 선택기와 목록 공유
  ...
  $("imgFile").addEventListener("change", () => {
    ...
    reader.onload = async () => {
      const res = await post("/api/stickers/upload", { name: f.name, data: reader.result });
      if (res.ok) { showMsg(`'${res.file}' 업로드 완료`); renderImages(); }
    };
    reader.readAsDataURL(f);
  });
  ...
  function chRow({dot, nm, tag, tagCls, piece, color, face, xBtn, dataAttr}) {
    const sw = [0, 1, 2, 3, 4, 5].map(c =>
      `<button type="button" class="pl-sw c${c}${color === c ? " sel" : ""}" data-c="${c}"></button>`).join("");
    return `<div class="ch-row" ${dataAttr || ""} data-color="${color}">
      ...
      <input type="text" class="ch-face-in" maxlength="4" value="${esc(face)}" placeholder="😊" title="말 위 이모지 (비우면 이름 첫 글자)">
  ...
  $("faceFile").addEventListener("change", () => {
    ...
    reader.onload = async () => {
      const res = await post("/api/stickers/upload", { name: f.name, data: reader.result });
      if (res.ok && facePickIdx !== null) {
        plList[facePickIdx].img = res.file;
        drawPlayers(); closeFacePick();
        showMsg(`'${res.file}' 업로드 완료 — [저장]을 눌러야 적용돼요.`);
      }
    };
    reader.readAsDataURL(f);
  });
