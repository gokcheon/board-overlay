...
(function () {
  ...
  // ── 칸 그림이 실제로 나갈 수 있는 거리 (fix43) ─────────────────────────────
  // 칸 밖으로 넘친 그림을 가리는 건 두 가지뿐이다.
  //   ① 화면 가장자리 — 보드 여백이 PADX 16 / PADY 32 라 그만큼만 보인다
  //   ② 보드 안쪽 판 — 전광판·상태바·타이머는 칸 그림보다 위 층(z 20)이라 그림을 덮는다
  // 이웃 칸끼리는 겹쳐도 되므로(겹침 순서로 처리) 제한이 아니다.
  // ★아래 좌표는 overlay.css 의 .scoreboard / .statusbar / .timerdock 과 세트다.
  //   내용이 가장 많이 찼을 때(효과 표시 + 6인 상태바 + 타이머 3개)를 실측해 넉넉히 잡았다.
  const PANEL_BOXES = [
    { part: "scoreboard", nm: "전광판", x: 320,  y: 258, w: 470, h: 520 },
    { part: "status",     nm: "상태바", x: 320,  y: 645, w: 740, h: 195 },
    { part: "timers",     nm: "타이머", x: 1080, y: 750, w: 340, h: 70  },
  ];
  // ※ 아래쪽 오버레이 설정 섹션에도 같은 뜻의 partOn 이 따로 있다 (이름 충돌 주의)
  const panelOn = key => !ovParts || ovParts[key] !== false;   // 값이 없으면 켜진 것

  // 칸 i 에서 네 방향으로 몇 px 까지 나갈 수 있는지 → {up,down,left,right} 각 {px, by}
  function tileRoom(i) {
    const [c, r] = pos[i];
    const M = TILE_IMG_MARGIN;
    const L = xs[c], T = ys[r], R = L + colW(c), B = T + rowH(r);
    const boxes = PANEL_BOXES.filter(p => panelOn(p.part))
      .map(p => ({ nm: p.nm, l: p.x, t: p.y, r: p.x + p.w, b: p.y + p.h }));
    const out = {};
    const put = (dir, px, by) => { out[dir] = { px: Math.max(0, Math.round(px)), by }; };
    // 화면 가장자리부터
    put("up", Math.min(M, T), T < M ? "화면 끝" : "");
    put("down", Math.min(M, H - B), (H - B) < M ? "화면 끝" : "");
    put("left", Math.min(M, L), L < M ? "화면 끝" : "");
    put("right", Math.min(M, W - R), (W - R) < M ? "화면 끝" : "");
    // 판이 더 가까우면 그쪽으로 교체
    for (const p of boxes) {
      const xHit = p.r > L - M && p.l < R + M;    // 가로로 겹치는 판만 위아래 판단에 쓴다
      const yHit = p.b > T - M && p.t < B + M;
      if (xHit && p.b <= T && T - p.b < out.up.px) put("up", T - p.b, p.nm);
      if (xHit && p.t >= B && p.t - B < out.down.px) put("down", p.t - B, p.nm);
      if (yHit && p.r <= L && L - p.r < out.left.px) put("left", L - p.r, p.nm);
      if (yHit && p.l >= R && p.l - R < out.right.px) put("right", p.l - R, p.nm);
    }
    return out;
  }
  ...
  function roomText(i) {
    ...
    const one = (nm, d) => {
      const v = rm[d];
      const full = v.px >= TILE_IMG_MARGIN;
      const cls = full ? "goldtext" : (v.px <= 10 ? "warntext" : "");
      return `<b class="${cls}">${nm} ${v.px}</b>${v.by ? `<span class="dimtext">(${v.by})</span>` : ""}`;
    };
    const tight = Object.entries(rm).filter(([, v]) => v.px < TILE_IMG_MARGIN && v.by && v.by !== "화면 끝");
    let s = `이 칸에서 그림이 나갈 수 있는 거리 — ` +
            `${one("위", "up")} · ${one("아래", "down")} · ${one("왼쪽", "left")} · ${one("오른쪽", "right")} (px)`;
    if (tight.length) {
      const nms = [...new Set(tight.map(([, v]) => v.by))].join("·");
      s += `<br><b class="warntext">${nms}</b>이(가) 그 방향을 덮어요 — 더 넘기고 싶으면
            <b class="goldtext">설정 → 오버레이 설정</b>에서 그 파츠를 끄고 OBS에 따로 배치하세요.`;
    }
    return s;
  }
  ...
  function renderBgRow() {
    ...
    if (editIndex !== null) hint += "<br>" + roomText(editIndex);   // fix43: 방향별 실제 여유
