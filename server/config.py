...
DEFAULTS = {
    ...
    # 칸 꾸미기 방식 (fix42) — "basic" | "image"
    #  basic = 지금까지 방식 (앱이 칸 바탕·테두리·이름 글자·카테고리 색 바를 그리고 스티커를 3자리에 얹음)
    #  image = 칸마다 넣은 통이미지(tile.bg_img) 한 장이 칸 전체를 대신함
    #          → 테두리 투명 · 카테고리 색 바 숨김 · 칸 이름 글자 안 그림 (이름 데이터 자체는 유지:
    #            전광판·벌칙 타이머·진행 기록이 계속 쓰기 때문. 공란으로 두면 타이머 중첩 판정이 깨진다)
    #          → 그림이 없는 칸은 자동으로 basic 으로 그림 (빈 구멍 방지)
    "tile_skin": "basic",
