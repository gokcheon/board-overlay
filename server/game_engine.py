...
class GameEngine:
        ...
        if t == "game_started":
            ...
            if d.get("rolls", 0) > 0:
                st["queue"].append({
                    ...
                    "ch": d.get("ch"),   # fix37: 합방 채널 태그 (표시용)
                })
            # rolls==0(기준 금액 미만)은 로그 보존만 하고 큐 미적재
        elif t == "guard_gift":
            gid = str(d.get("id") or f'{d.get("nick", "?")}-{d.get("ts", 0)}')
            auto = d.get("auto_piece")       # fix37 합방: 후원 터진 채널의 말에게 자동 귀속 (곡천 확정)
            if auto is not None and 0 <= int(auto) < len(st["players"]):
                st["piece_guards"][int(auto)] += 1
                st["guard_gifters"][int(auto)].append(d["nick"])
            elif len(st["players"]) <= 1:    # 1인: 지정 단계 생략, 즉시 귀속
                st["piece_guards"][0] += 1
                st["guard_gifters"][0].append(d["nick"])
            else:                            # 다인(단일 모드): 스트리머가 실시간 지정 (fix13 지목제)
    ...
    def _land(self, st: dict, idx: int, nick: str, piece: int = 0) -> None:
        ...
        for op in tile.get("ops", []):
            ...
            if k == "counter_add":
                ...
                pass  # fix40: '주사위 두배로' — 즉시 이동으로 굴림 계산(cmd_next)에서 처리 (버프 폐지)
            elif k == "extra_roll":
                st["queue"].insert(0, {
    ...
    def cmd_next(self) -> dict:
        ...
        if outcome.get("special"):
            ...
            if head.get("ch"):
                data["ch"] = head["ch"]   # fix37: 채널 태그 전파
            if redo is not None:
                data["redone"] = True
        ...
        segments = []
        path, cur = self._walk(st["positions"][piece], direction, value)
        segments.append({"path": path, "to": cur})
        final = self._chain(cur, direction, segments)
        # fix40: '주사위 두배로' 칸 — 밟으면 같은 값으로 즉시 한 번 더 이동 후 다음 차례 (곡천 확정).
        #  예전 룰(다음 굴림 ×2 버프)은 폐지. 두배 칸이 연달아 이어지는 극단 케이스는 상한으로 보호.
        again = 0
        for _ in range(CHAIN_LIMIT):
            if not any(o.get("op") == "dice_multiplier"
                       for o in self._tile(final).get("ops", [])):
                break
            again += 1
            path, cur = self._walk(final, direction, value)
            segments.append({"path": path, "to": cur})
            final = self._chain(cur, direction, segments)
        data = {
            "id": head["id"], "nick": head["nick"], "value": value, "moved": value,
            "x2_used": False, "again": again, "piece": piece, "segments": segments,
            "final": final, "ts": time.time(),
        }
        if head.get("ch"):
            data["ch"] = head["ch"]       # fix37: 채널 태그 전파
        if redo is not None:
