...
class TokenStore:
    ...
    fix37: path 지정 가능 — 합방 채널별 토큰(collab/ 폴더)도 같은 저장소 클래스로."""

    def __init__(self, path=None):
        self.path = path or (data_dir() / "chzzk_tokens.json")
    ...
    def save(self, access: str, refresh: str, extra: dict = None) -> None:
        data = {
            "access_token": access,
            "refresh_token": refresh,
            "issued_at": time.time(),
        }
        if extra:                       # fix37: 합방 토큰엔 채널 정보를 함께 보관
            data.update(extra)
...
class ChzzkClient:
    ...
    def __init__(self, cfg: dict, on_donation=None, on_chat=None, on_status=None, token_path=None):
        self.cfg = cfg
        ...
        self.tokens = TokenStore(token_path)   # fix37: 합방 채널별 인스턴스 지원
