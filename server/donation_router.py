...
class DonationRouter:
    ...
    def on_donation(self, donation_id: str, nick: str, amount: int) -> dict:
        ...
        if amount == int(self.cfg.get("guard_price", 10000)):  # 정확 일치만
            self.engine.record("guard_gift", {
                "id": donation_id, "nick": nick, "amount": amount, "ts": time.time(),
            })
            return {"kind": "guard", "nick": nick}
        ...
        self.engine.record("donation_received", {
            "id": donation_id, "nick": nick, "amount": amount,
