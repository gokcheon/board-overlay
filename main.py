...
from server import branding, imgsize, updater
...
def load_board() -> list:
    ...
    tiles = json.loads(user_board.read_text(encoding="utf-8"))
    # fix40: '주사위 ×2' 룰 변경(같은 값 즉시 한 번 더) — 옛 기본 표기를 쓰던 보드는 이름만 이관.
    #  표시 문구만 바꾸므로 이벤트·리플레이와 무관 (직접 바꾼 커스텀 이름은 건드리지 않음).
    changed = False
    for t in tiles:
        if any(o.get("op") == "dice_multiplier" for o in t.get("ops", [])):
            if t.get("label") == "주사위 ×2":
                t["label"] = "주사위 두배로"
                changed = True
            if t.get("rule_text") == "다음 굴림 이동칸 2배!":
                t["rule_text"] = "밟은 굴림 값만큼 즉시 한 번 더!"
                changed = True
    if changed:
        atomic_write_json(user_board, tiles)
    return tiles
...
def public_config() -> dict:
    ...
    return {
        ...
        # fix42 — 칸 꾸미기 방식 (basic | image). 오버레이가 이 값으로 렌더를 갈아탄다.
        "tile_skin": "image" if cfg.get("tile_skin") == "image" else "basic",
        ...
        if cmd == "start":
            ...
            if result["kind"] == "roll":
                ...
                if d.get("special"):
                    ...
                    ag = (f' — 두배 칸! 같은 값으로 {d["again"]}번 더'
                          if d.get("again") else "")   # fix40
                    logs.append(f'{who}{d["nick"]} 굴림 {d["value"]}{x2}{ag} → {label}{redo}')
            else:
                logs.append("✓ 칸 효과 완료")
        elif cmd == "choose":
...
@app.post("/api/tileskin")
async def api_tileskin(payload: dict):
    """칸 꾸미기 방식 전환 (fix42) — basic | image. 표시 전용이라 진행 중에도 바꿀 수 있다."""
    skin = "image" if str((payload or {}).get("skin", "")) == "image" else "basic"
    cfg["tile_skin"] = skin
    save_config(cfg)
    await hub.broadcast({"type": "tileskin", "tile_skin": skin})
    await log_event("🖼 칸 꾸미기 방식 — " + ("칸 통이미지" if skin == "image" else "기본"))
    return {"ok": True, "tile_skin": skin}


@app.get("/api/imgmemory")
async def api_imgmemory():
    """오버레이가 실제로 쓰는 그림들의 예상 메모리 (fix42) — 설정 창 표시용."""
    return {"ok": True, **_overlay_image_memory()}
...
async def api_mode(payload: dict):
    ...
    if mode == "collab":
        ...
        styles = [{"color": int(c.get("my_color", 0)) % 6, "face": str(c.get("my_face", ""))[:4],
                   "img": str(c.get("my_img", ""))[:120]}]      # fix40: 합방도 말 그림 지원
        for m in coll.get("members", []):
            ...
            m["img"] = str(upd.get("img", m.get("img", "")))[:120]   # fix40
            styles.append({"color": m["color"], "face": m["face"], "img": m["img"]})
...
def _sticker_usage() -> dict:
    ...
    for i, t in enumerate(engine.board):
        if t.get("bg_img") and t["bg_img"] not in used:      # fix42: 칸 통이미지
            used[t["bg_img"]] = f"{i}번 칸 그림"
...
def _overlay_image_memory() -> dict:
    """오버레이가 '실제로 화면에 띄우는' 그림들의 예상 메모리 (fix42).

    OBS 브라우저 소스는 그림을 표시 크기가 아니라 원본 픽셀 수만큼 메모리에 펼친다.
    그래서 파일 용량이 아니라 (가로 × 세로 × 4바이트) 를 합산한다.
    보관함에만 있고 어디에도 안 쓰인 그림은 로드되지 않으므로 세지 않는다.
    같은 파일을 여러 칸에 재사용하면 브라우저가 한 번만 디코드하므로 중복 제거한다.
    """
    used = _sticker_usage()                       # 파일명 → 사용처 (중복 제거된 집합)
    items, total = [], 0
    for fname in sorted(used):
        size = imgsize.image_size(STICKER_DIR / fname)
        by = imgsize.pixel_bytes(size)
        total += by
        items.append({"file": fname, "w": (size or [0, 0])[0], "h": (size or [0, 0])[1],
                      "bytes": by, "where": used[fname]})
...
async def api_stickers_list():
    ...
    # fix42: 파일별 픽셀 크기 (이미지 관리 탭의 크기·예상 메모리 표시용)
    sizes = {}
    for p in files:
        wh = imgsize.image_size(p)
        if wh:
            sizes[p.name] = [wh[0], wh[1]]
    return {"files": [p.name for p in files], "usage": _sticker_usage(),
            "sizes": sizes, "memory": _overlay_image_memory()}


@app.post("/api/stickers/upload")
async def api_stickers_upload(payload: dict):
    """데이터URL(base64)로 업로드 — 추가 패키지 없이 처리 (fix41: 20MB 제한).

    조작앱(control.js)이 보내기 전에 긴 변 512px 로 자동 축소하므로 실제로
    도착하는 데이터는 훨씬 작다. 여기 20MB 는 조작앱을 거치지 않고 API 를
    직접 호출한 경우까지 막기 위한 상한이다.
    """
    import base64
    payload = payload or {}
    ...
    if len(raw) > 20 * 1024 * 1024:
        return {"ok": False, "msg": "그림이 20MB를 넘어요. 조금 줄여서 올려 주세요."}
...
async def api_stickers_delete(payload: dict):
    ...
    bg_used = [i for i, t in enumerate(engine.board) if t.get("bg_img") == fname]   # fix42
    if bg_used:
        tiles = ", ".join(f"{i}번" for i in bg_used[:5])
        return {"ok": False, "msg": f"{tiles} 칸의 칸 그림으로 쓰고 있어요. 먼저 바꾸고 삭제해 주세요."}
