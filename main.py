...
async def handle_chzzk_donation(payload: dict) -> None:
    """DONATION 이벤트. 텍스트(CHAT)·영상(VIDEO) 후원 모두 굴림으로 인정.
    ...
    if result["kind"] == "guard":
        ...
        if len(engine.state["players"]) > 1:
            await log_event(f"💝 {nick}님이 방어권을 선물! ({amount:,}원) — 누구에게 줄지 지정해 주세요")
        else:
            await log_event(f"💝 {nick}님이 방어권을 선물! ({amount:,}원)")
    elif result["kind"] == "rolls":
        await log_event(f'💰 {nick} {amount:,}원 → {result["rolls"]}굴림 대기')
    elif result["kind"] == "ignored":
        await log_event(f"💤 {nick} {amount:,}원 — 기준 금액 미만 (기록만)")
    await push_state()
