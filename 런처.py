...
try:                                              # fix28: 버전은 server/version.py 한 곳
    ...
    VERSION = "fix40"
try:                                              # fix38: 크레딧·채널 주소는 server/branding.py 한 곳
    from server import branding as BR
except Exception:
    BR = None
...
# ── Tailscale (fix39) — 합방 상대 OBS에 오버레이를 보여줄 때 쓰는 무료 개인 VPN
def tailscale_dir():
    for base in (os.environ.get("ProgramFiles"), os.environ.get("ProgramFiles(x86)")):
        if base and (Path(base) / "Tailscale" / "tailscale-ipn.exe").exists():
            return Path(base) / "Tailscale"
    return None


def tailscale_ip():
    """로그인돼 있으면 내 Tailscale 주소(100.x.x.x)를 돌려준다. 아니면 None."""
    d = tailscale_dir()
    if not d:
        return None
    try:
        out = subprocess.run([str(d / "tailscale.exe"), "ip", "-4"],
...
def draw(sv: Server, note: str):
    ...
    if BR:  # fix38: 1주년 크레딧 + 고지 한 줄
        o.append(f"  {DIM}치지직 스트리머 {YEL}{BR.CHANNEL_OWNER}{RST}{DIM} 방송 1주년 기념 팬메이드 · 무단 재배포·수정·판매 금지{RST}")
    o.append("  " + LINE)
    if on:
    ...
    o.append("")
    o.append(f"   {LAV}조작앱{RST} {DIM}(내 컴퓨터·폰){RST}   http://{ip}:{PORT}/control")
    ...
    o.append(f"   {YEL}[4]{RST} 이 컴에서 조작앱 열기   {YEL}[5]{RST} 패키지 점검   {YEL}[8]{RST} 사용 설명서")
    o.append(f"   {YEL}[6]{RST} Tailscale 실행 {DIM}(합방 오버레이 공유){RST}   {YEL}[7]{RST} Tailscale 설치")
    ch_btn = f"   {YEL}[9]{RST} {GRN}{BR.CHANNEL_OWNER} 치지직 채널 놀러가기{RST}   " if BR else "   "
    o.append(ch_btn + f"{YEL}[0]{RST} 종료")
    o.append("  " + LINE)
...
def main():
    ...
    while True:
        ...
        elif key == "6":             # fix39: Tailscale 실행 (합방 오버레이 공유)
            d = tailscale_dir()
            if not d:
                note = "Tailscale이 설치되어 있지 않아요 — [7]을 눌러 설치해 주세요."
            else:
                try:
                    os.startfile(str(d / "tailscale-ipn.exe"))  # 트레이 앱 실행
                except Exception:
                    pass
                ts = tailscale_ip()
                if ts:
                    url = f"http://{ts}:{PORT}/overlay"
                    note = (f"합방 상대 OBS용 주소를 복사했어요 → {url}" if copy(url)
                            else f"합방 상대 OBS용 주소: {url}")
                else:
                    note = "Tailscale을 켰어요 — 트레이 아이콘에서 로그인하고 [6]을 다시 누르면 합방용 주소를 복사해 드려요."
        elif key == "7":             # fix39: Tailscale 설치 페이지
            import webbrowser
            webbrowser.open("https://tailscale.com/download/windows")
            note = "Tailscale 다운로드 페이지를 열었어요 — 설치 후 [6]으로 실행하면 돼요."
        elif key == "8":             # fix39: 사용 설명서(메뉴얼)
            manual = ROOT / "메뉴얼.html"
            if manual.exists():
                try:
                ...
                note = "사용 설명서를 브라우저로 열었어요."
            else:
                note = "메뉴얼.html 파일이 없어요 — 재설치하면 생겨요."
        elif key == "9" and BR:      # fix38: 김띵띵띵 채널 놀러가기
            import webbrowser
            webbrowser.open(BR.CHANNEL_URL)
            note = f"{BR.CHANNEL_OWNER} 채널을 브라우저로 열었어요 💚"
        elif key == "5":
            miss = missing_packages()
            if miss:
                install_packages()
